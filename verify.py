...
with sync_playwright() as p:
    ...
    pg.evaluate("document.getElementById('btn-save-manual').click()"); pg.wait_for_timeout(300)
    check("set_created", pg.evaluate("document.querySelectorAll('.card-row').length") == 1)
    pg.evaluate("document.querySelector('[data-mode=flashcards]').click()"); pg.wait_for_timeout(250)
    ...
    f = pg.evaluate("(JSON.parse(localStorage.getItem('recall_v3')).sets[0].cards.find(c=>c.f)||{}).f||{}")
    check("fsrs_persists", f.get("stability", 0) > 0 and f.get("reps", 0) > 0, f.get("stability"))
    pg.evaluate("document.getElementById('btn-undo').click()"); pg.wait_for_timeout(200)
    check("undo_works", pg.evaluate("document.getElementById('study-count').textContent") == "1/1")
    pg.evaluate("document.getElementById('btn-exit-study').click()"); pg.wait_for_timeout(250)
    pg.evaluate("document.getElementById('sum-home') && document.getElementById('sum-home').click()"); pg.wait_for_timeout(200)
    pg.evaluate("document.querySelector('.set-row').click()"); pg.wait_for_timeout(250)
    pg.once("dialog", lambda d: d.accept())
    pg.evaluate("document.getElementById('btn-delete-set').click()"); pg.wait_for_timeout(250)
